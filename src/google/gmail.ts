import { DEFAULT_ACCOUNT, getGoogleAccessToken, listGoogleAccounts } from "./auth.ts";
...
async function callApi<T>(path: string, init: RequestInit = {}, account: string = DEFAULT_ACCOUNT): Promise<T> {
	let lastBody = "";
	let lastStatus = 0;
	...
	throw new Error(`Gmail API ${lastStatus} on ${path} [account=${account}]: ${lastBody.slice(0, 300)}`);
}
...
// List from a single account.
async function listMessagesFor(account: string, opts: { q?: string; maxResults?: number }): Promise<EmailSummary[]> {
	const params = new URLSearchParams({ maxResults: String(opts.maxResults ?? 20) });
	if (opts.q) params.set("q", opts.q);
	const list = await callApi<{ messages?: { id: string; threadId: string }[] }>(`/messages?${params}`, {}, account);
	const ids = (list.messages ?? []).slice(0, opts.maxResults ?? 20);
	const summaries = await mapWithConcurrency(ids, LIST_CONCURRENCY, (m) => getMessageRaw(m.id, "metadata", account));
	return summaries.map((m) => toSummary(m, account));
}

// Default account = "all": merge from every configured account.
// If only one is configured, it's effectively single-account.
export async function listMessages(opts: {
	q?: string;
	maxResults?: number;
	account?: string;
}): Promise<EmailSummary[]> {
	const accounts = !opts.account || opts.account === "all" ? listGoogleAccounts() : [opts.account.toLowerCase()];
	if (accounts.length === 0) return [];
	const perAccount = await Promise.all(accounts.map((a) => listMessagesFor(a, opts)));
...
async function getMessageRaw(id: string, format: "metadata" | "full", account: string): Promise<RawMessage> {
	...
	return callApi<RawMessage>(`/messages/${id}?${params}`, {}, account);
}

function toSummary(m: RawMessage, account: string): EmailSummary {
	return {
		account,
		id: m.id,
		threadId: m.threadId,
...
export async function readEmail(
	...
	const full = await getMessageRaw(id, "full", account);
	return { ...toSummary(full, account), body: extractBody(full.payload) };
}
...
export async function createDraft(input: DraftReplyInput): Promise<{ id: string; messageId: string }> {
	...
	const data = await callApi<{ id: string; message: { id: string } }>(
		"/drafts",
		{
			method: "POST",
...
export async function sendDraft(
	...
	return callApi("/drafts/send", { method: "POST", body: JSON.stringify({ id: draftId }) }, account);
}

export async function sendNew(input: {
	to: string;
	subject: string;
	body: string;
	account?: string;
	...
	const account = input.account ?? DEFAULT_ACCOUNT;
	const raw = encodeBase64Url(buildRfc822(input));
	return callApi("/messages/send", { method: "POST", body: JSON.stringify({ raw }) }, account);
}
...
// Cache labels per account — system label IDs are constant but user labels are
// per-account.
const labelCache = new Map<string, GmailLabel[]>();

export async function listLabels(account: string = DEFAULT_ACCOUNT): Promise<GmailLabel[]> {
	const key = account.toLowerCase();
	const cached = labelCache.get(key);
	if (cached) return cached;
	const data = await callApi<{ labels?: GmailLabel[] }>("/labels", {}, account);
	const labels = data.labels ?? [];
	labelCache.set(key, labels);
	return labels;
}

// Resolve label NAMES to label IDs (case-insensitive). Unknown names are
